...
if(!$conn)
{
    echo "failed to connect ";
        ...
        $c = '0';
        $sql = "SELECT * FROM `collection_edb` WHERE `status` = 'pending' AND `waste_quantity` = '$c' ";
...
<head>
    ...
    <title>Collection Quantity Update</title>
...
<div class="container">
<br />
    <h2 align="center">Update Quantity</h2>
    <br />
</div>
...
<?php if ($wastecat == "EdibleFood"): ?>
        ...
        <td><a href="quantity.php?collect=<?php echo $collect; ?>" class="btn btn-info" role="button">Approve</a></td>
        <td><a href="rejquantity.php?collect=<?php echo $collect; ?>" class="btn btn-danger" role="button">Reject</a></td>
        </tr>
        <?php
        }mysqli_close($conn);
        ?>
...
<?php elseif ($wastecat == "InedibleFood"): ?>
    <div class="table-responsive">
        ...
        <?php
        $c = '0';
        $sql = "SELECT * FROM `collection` WHERE `status` = 'pending' AND `waste_quantity` = '$c' ";
        $query=mysqli_query($conn,$sql);
        while($row = mysqli_fetch_assoc($query))
        {
