...
$qquery = "SELECT `quantity` FROM `slurry_request` WHERE `status` = 'pending' AND `code` = '$code'";
$qsql=mysqli_query($conn,$qquery);
if(mysqli_num_rows($qsql) == 0)
{
    echo "<SCRIPT>
    alert('No pending request')
    window.location.replace('srequestlist.php')</script>";
    // header("location:srequestlist.php");   
}
while($row = mysqli_fetch_assoc($qsql))
{
    $askquantity1=$row['quantity'];
}
...
<head>
    ...
    <title>Slurry Request Approval</title>
...
<div class="container"></div>
    ...
    <div class="container" align="center">
        <h2>Approve slurry request</h2>
        </div>
        <br/>
        <form class="form-horizontal" method="post" action="">
			<div class="form-group">
			  <label class="col-sm-5 control-label">Distribution serial no. </label>
			  <div class="col-xs-5">
				<input type="text" class="form-control" name="dsrno" required="" .col-xs-3/>
				<!-- <input type="email" class="form-control" id="inputEmail3" placeholder="Email"> -->
			  </div>
			</div>
			<div class="form-group">
			  <label class="col-sm-5 control-label">Fcode </label>
			  <div class="col-xs-5">
				<input type="text" name="fcode" readonly required="" value="<?php echo $code;?>" required="" class="form-control" .col-xs-3/>
				
			  </div>
			</div>
			<div class="form-group">
				<label class="col-sm-5 control-label">Asked quantity </label>
				<div class="col-xs-5">
				  <input type="number" readonly name="askquantity" value="<?php echo $askquantity1; ?>" required="" class="form-control" .col-xs-3/>
              ...
              <div class="form-group">
				...
				<div class="col-xs-5">
				  <input type="number" name="appq" required="" class="form-control" .col-xs-3/>
				  
				</div>
			  </div>
			<div class="form-group">
			  <div class="col-sm-offset-5 col-sm-10">
				<input type="submit" class="btn btn-primary"name="sub" value="Approve"/> 
			  </div>
			</div>
		  </form>
...
$approved=$_POST['appq'];
$sql="SELECT * FROM `total_slurry`";
$query=mysqli_query($conn,$sql);
while($row = mysqli_fetch_assoc($query))
{
    $b=$row['bcode'];
    $quantity = $row['slurry_quantity'];
    if($approved<=$quantity)
    {
        $new_quantity = $quantity - $approved;
          if($new_quantity == 0 || $new_quantity<0)
        {
            $sql5 = "DELETE FROM `total_slurry` WHERE `bcode`='$b' ";
        ...
        if($new_quantity > 0)
        {
        $sql2 = "UPDATE `total_slurry` SET `slurry_quantity`='$new_quantity' WHERE `bcode` = '$b' ";
        $query2=mysqli_query($conn,$sql2);
        if(!$query)
        {
            echo "Cannot update slurry quantity";
        }
         }
        break;
    }
    if($quantity == 0)
    {
        $sql4 = "DELETE FROM `total_slurry` WHERE bcode=$b ";
...
//{
    ...
    */$dsrno=$_POST['dsrno'];
    ...
    //fetching farmer details from slurry_request table
    $fquery="SELECT `code`, `name`, `phone`, `address`, `date` FROM `slurry_request` WHERE`code` = '$code'";
    ...
    //fertching the details from bio-plant table
    $bquery="SELECT `code`,`Name`,`address`,`email`,`phone`,`slurry_price` FROM bio_plant WHERE `code` = '$b'";
    ...
    //inserting the details in the distribution table
    $dquery="INSERT INTO `distribution`(`distribute_srno`, `fcode`, `fname`, `fphone`, `faddress`, `approved_quantity`, `Rdate`, `bcode`, `bname`, `bphone`, `bemail`, `baddress`, `slurry_price`, `Adate`) VALUES ('$dsrno','$f1','$f2','$f3','$f4','$approved','$f5','$b1','$b2','$b5','$b4','$b3','$b6','$date')";
    if($dresut=mysqli_query($conn,$dquery))
    {
        $query="UPDATE `slurry_request` SET `status`='processed' WHERE `code` = '$code'";
