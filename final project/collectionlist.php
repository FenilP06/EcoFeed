...
<div class="container">
<br />
    <h2 align="center">Update Collection Record</h2>
    <br />
</div>
 <!-- edible -->
 <?php if ($wastecat == "EdibleFood"): ?>
        <div class="table-responsive">
            ...
            <?php
            $sql2 = "SELECT `collectedb_srno`, `date`, `dcode`, `dname`, `vcode`, `vname`, `tcode`, `tname`, `dist_loc`,`appx_quantity`, `waste_quantity`, `status` FROM `collection_edb`";
            $query1=mysqli_query($conn,$sql2);
            while($row = mysqli_fetch_assoc($query1))
            {
                $collect = $row['collectedb_srno'];
            ?>
            <tr>
            ...
            <td><?php echo $row['dist_loc'] ?></td>
            <td><?php echo $row['appx_quantity'] ?></td>
            <td><?php echo $row['waste_quantity'] ?></td>
            <td><?php echo $row['status'] ?></td>
            <td><a href="collectionedit.php?collect=<?php echo $collect?>" class="btn btn-info" role="button">Edit</a></td>
            <td><a href="collectiondeledit.php?collect=<?php echo $collect?>" class="btn btn-danger" role="button">Delete</a></td>
            </tr>
            <?php
            } mysqli_close($conn);
            ?>
    ...
    <?php elseif ($wastecat == "InedibleFood"): ?>        
        ...
        <div class="table-responsive">
            ...
            <th>App_Qty</th>
            <th>Waste_Quantity</th>
            <th>Slurry_Quantity</th>
            <th>Total_Waste_price</th>
            <th>Total_slurry_price</th>
            <th>Status</th>
            <th>Edit</th>
            <th>Delete</th>
            </tr>
            <?php
            $sql = "SELECT `collect_srno`, `date`, `dcode`, `dname`, `vcode`, `vname`, `tcode`, `tname`, `bcode`, `bname`, `appx_quantity`, `waste_quantity`, `slurry_quantity`, `twaste_price`, `tslurry_price`, `status` FROM `collection`";
            $query=mysqli_query($conn,$sql);
            while($row = mysqli_fetch_assoc($query))
            {
                $collect = $row['collect_srno'];
            ?>
            <tr>
            <td><?php echo $collect ?></td>
            <td><?php echo $row['date'] ?></td>
